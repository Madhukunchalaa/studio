...
/*
|--------------------------------------------------------------------------
| AI Video Production Form Handler
|--------------------------------------------------------------------------
*/
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name) || empty($email)) {
        header("Location: ai-video-production.html?status=empty");
        exit;
    }
    ...
    $save_result = save_lead([
        ...
        'phone' => $phone,
        'service' => $service,
        'source' => 'AI Video Production' // Specific source
    ]);

    if (!$save_result) {
        error_log(date('[Y-m-d H:i:s]') . " DB ERROR: Failed to save AI Video lead for $email\n", 3, $log_file);
    }
    ...
    <body>
        <div class='container'>
            <div class='header'>
                ...
                <h1 class='title'>AI Video Production Inquiry</h1>
            </div>
    ...
    if (mail($to, $subject, $email_content, $headers)) {
        error_log(date('[Y-m-d H:i:s]') . " SUCCESS: AI Video email sent for $email\n", 3, $log_file);
        header("Location: thank-you.html?source=ai-video");
        exit;
    } else {
    ...
    header("Location: ai-video-production.html");
    exit;
}
?>